...
| GitLab type | Self-hosted NatWest, AWS-hosted Kubernetes runners | 2026-06-14 |
...
| Java version | **OpenJDK 21** (virtual threads, NatWest confirmed) | 2026-06-14 |
...
| Why NOT MS SQL | MS SQL requires paid licence — contradicts £0 cost mission. NatWest uses MS SQL centrally but Mockingbird is a new platform. PostgreSQL is free. | 2026-06-14 |
...
| Secrets management | **HashiCorp Vault (primary)** — NatWest team uses it aggressively | 2026-06-14 |
...
| Decision | Confirmed Value | Date |
|----------|----------------|------|
| Artifactory base URL | `https://artifactory-server.app.banksvcs.net/artifactory/dws-all-repos` | 2026-06-14 |
...
| ⚠️ CRITICAL CONSTRAINT | Artifactory (`banksvcs.net`) is on internal NatWest network — **NOT accessible from AWS EC2** | 2026-06-14 |
| Build consequence | ALL Maven/pip/npm builds happen in GitLab CI only. EC2 only runs pre-built Docker images. | 2026-06-14 |
| Docker base images | From GitLab Container Registry (NatWest mirrors) — accessible from both GitLab CI AND EC2 | 2026-06-14 |

### GitLab Registry & CI

| Decision | Confirmed Value | Date |
|----------|----------------|------|
| GitLab Container Registry URL | `registry.natwest.gitlab-dedicated.com` | 2026-06-14 |
| Java 21 base image (full path) | `registry.natwest.gitlab-dedicated.com/natwestgroup/engineeringartifacts/engineeringcomponents/executors/build/natwest-java/java-21.0.7_6-ubi9-9.6-1751286687:latest` | 2026-06-14 |
| Kaniko builder image | `registry.natwest.gitlab-dedicated.com/natwestgroup/engineeringartifacts/engineeringcomponents/executors/build/kaniko/multi-arch/v1.24:v1.24.0-debug` | 2026-06-14 |
...
| CI includes | `Container-Scanning.gitlab-ci.yml` + `Secret-Detection.gitlab-ci.yml` (from `natwestgroup/EngineeringArtifacts/.../continuous-integration@5.0.2`) | 2026-06-14 |
...
| Phase 2 auth | LDAP — NatWest network login | 2026-06-14 |
| LDAP group format | `memberOf: CN=SV-Team,OU=Groups,DC=natwest,DC=com` | 2026-06-14 |
...
| Decision | Confirmed Value | Date |
|----------|----------------|------|
| Application logs | **Splunk** (existing NatWest) via CloudWatch Logs → Splunk HEC | 2026-06-14 |
| Log format | Structured JSON to stdout; collected by CloudWatch | 2026-06-14 |
| APM / tracing | **AppDynamics** (existing NatWest) — Java agent in stub containers | 2026-06-14 |
...
| Other tools in NatWest | DX APM, Elasticsearch also available but not primary for Mockingbird | 2026-06-14 |
...
| NatWest branding | YES — all PDF and PPT reports must use NatWest logo/colours/template | 2026-06-14 |
...
|----|----------------|-------------------|------|
| C1 | GitLab Container Registry URL | `registry.natwest.gitlab-dedicated.com` ✅ | 2026-06-14 |
| C2 | Artifactory URL (Maven) | `https://artifactory-server.app.banksvcs.net/artifactory/dws-all-repos` ✅ | 2026-06-14 |

### 🔴 STILL BLOCKING

| ID | What We Need | Impact if Missing | Who Provides |
|----|-------------|------------------|-------------|
| C3 | **Is PostgreSQL acceptable** for a new NatWest application, or is MS SQL mandated for all apps? | If MS SQL mandatory, switch DB before any table is created. Phase 1 (CLI) does NOT need this. | User — check with architecture/DBA team |
...
| I3 | **Splunk HEC endpoint** + **token** e.g. `https://splunk.natwest.internal:8088` + token | Log forwarding from CloudWatch to existing Splunk | User — check with Splunk/logging team |
...
| I5 | **LDAP server hostname + port** + **base DN** e.g. `ldap.natwest.internal:389`, `DC=natwest,DC=com` | LDAP authentication (Phase 2 auth service) | User — will provide when ready |
...
|----|-------------|--------|
| U1 | **NatWest branding assets**: official logo (PNG/SVG), brand hex colours, fonts, PowerPoint template | PDF and PPT report generation |
| U2 | **NatWest internal CA certificate** (PEM format) | HTTPS on stub servers using internal CA (avoids self-signed cert warnings) |
...
| Docker build tool | Kaniko (not DinD) | k8s runners don't allow privileged containers in NatWest |
| Container registry | GitLab Container Registry | NatWest already uses GitLab; avoids ECR cost |
...
| Question | Answer |
|----------|--------|
| Does NatWest use Artifactory? | YES — Maven, PyPI, npm, Docker all mirrored internally |
| Java version? | OpenJDK 21+ |
| GitLab hosted or cloud? | Self-hosted, centrally managed at NatWest |
...
| NatWest branding on reports? | YES |
...
| LDAP format? | `memberOf: CN=SV-Team,OU=Groups,DC=natwest,DC=com` |
...
| GitLab registry URL? | `registry.natwest.gitlab-dedicated.com` |
| Artifactory URL? | `https://artifactory-server.app.banksvcs.net/artifactory/dws-all-repos` |
| Artifactory credentials format? | Env vars ARTIFACTORY_USR + ARTIFACTORY_PSWD (fetched from Vault in CI) |
| Can Artifactory be reached from EC2? | NO — banksvcs.net is internal NatWest network only |
...
| Java 21 Docker base image? | registry.natwest.gitlab-dedicated.com/.../natwest-java/java-21.0.7_6-ubi9-9.6-1751286687:latest |
| Kaniko image? | registry.natwest.gitlab-dedicated.com/.../kaniko/multi-arch/v1.24:v1.24.0-debug |
