...
def _run_loop_once(process_message_side_effect):
    """Drive run_loop for exactly one message, then stop it."""
    ...
    with patch("deployer_worker.worker.process_message", side_effect=process_message_side_effect):
        with pytest.raises(_StopLoop):
            run_loop(
                sqs_client, db_factory, MagicMock(), GITLAB_PROJECT_ID, GITLAB_REGISTRY,
